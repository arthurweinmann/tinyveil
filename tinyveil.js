...
function AssertInstOf(target, ...src) {
    for (let i = 0; i < src.length; i++) {
        if (!(src[i] instanceof target)) {
            panic("invalid parameter ", i, ": we got ", ((src[i].constructor && src[i].constructor.name) ? src[i].constructor.name : src[i]), " but expected ", ((target.constructor && target.constructor.name) ? target.constructor.name : target));
        }
    }
}
